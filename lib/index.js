...
exports.plugin = {
  ...
  register: async (server, options) => {
    ...
    const routeModifiers = server.config || _.get(server, 'realm.modifiers');
    ...
    const internals = {
      handler (request, h) {
        const hapiSwaggeredSettings = _.get(server, 'plugins.hapi-swaggered.settings');
    ...
    server.route([settings.path || '/', `${routePath}/index.html`].map((path) => {
      return {
        method: 'GET',
        path: path,
        options: {
    ...
    server.route([`${routePath}/{path}`, `${routePath}/{path*2}`].map((path) => {
      return {
        method: 'GET',
        path: path,
        options: {
